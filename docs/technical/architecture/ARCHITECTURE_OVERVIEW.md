...
5. **Blender is not on the parse/IR path.** Runtime remains `converter → catalog/library → reusable part`. When STATE records S2C-11.7.1 or S2C-11.8.1, demand-driven generation of an eligible SDK-mesh identity may invoke Blender as a conversion step; assembly then inserts the generated `{geometry_id}.SLDPRT`. That is not a general Blender runtime stage.
6. **Geometry classes differ.** CubeTopology armor can often be native CAD. TriangleMesh functional/detail blocks may need source/model assets and a different library recipe. Do not force one production mechanism. A completed TriangleMesh library remains cold storage, outside M7–M15 executable scope, except the packaged S2C-11.7.1 bind and the demand-driven S2C-11.8.1 / S2C-11.11.1 / S2C-11.15.1 Large Grid vanilla TriangleMesh resolver recorded in STATE. That is not universal vanilla support. Empty-subtype resolution stays a bounded TypeId-plus-grid uniqueness rule, not a general empty-subtype grant.
...
8. **OS / tool split.** Space Engineers and the ModSDK stay on Linux as authoring and verification evidence. SolidWorks 2026 stays in the Windows VM. The Windows SolidWorks-backed path operates from a repository clone, an operator-supplied `bp.sbc`, the repository catalog and recipes, Python, pywin32, and SolidWorks 2026. Parse, catalog load, and IR do not require a Space Engineers install. When STATE records S2C-11.8.1, preflight and policy may resolve catalog-unknown vanilla identities against an operator-configured game-content root; generating those SDK-mesh parts also requires the official ModSDK content root and Blender. Packaged-catalog conversion remains install-free when that root is unset. Linux and Windows may use separate Git clones; the operator synchronizes them. Blueprint transfer is operator-managed. Linux-to-Windows automation and remoting are out of scope. Details: [ADR-003](../adr/ADR-003_SOLIDWORKS_BACKEND.md).
