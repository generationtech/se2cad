...
5. **Blender is not on the parse/IR path.** Runtime remains `converter → catalog/library → reusable part`. When STATE records S2C-11.7.1, demand-driven generation of one authorized SDK-mesh identity may invoke Blender as a library-build conversion step; assembly then inserts the generated `{geometry_id}.SLDPRT`. That is not a general Blender runtime stage.
6. **Geometry classes differ.** CubeTopology armor can often be native CAD. TriangleMesh functional/detail blocks may need source/model assets and a different library recipe. Do not force one production mechanism. A completed TriangleMesh library remains cold storage, outside M7–M15 executable scope, except the one authorized S2C-11.7.1 bind recorded in STATE. M11 may classify and except other such blocks.
